...
// Reduce kernel that uses shared memory
__global__ void shared_reduce_kernel(float *d_out, float *d_in){
	...
	for (unsigned int s = blockDim.x / 2; s > 0; s >>= 1){
		...
		// add A[i] + B[i + len(A)], i.e. Add the value from the corresponding location in other half
		if (threadId < s){
			s_data[threadId] += s_data[threadId + s];  // Reading and writing to shared memory
		}
...
void reduce(float *d_out, float *d_intermediate, float *d_in, int size, bool usesSharedMemory){
	...
	if (usesSharedMemory){
		...
		shared_reduce_kernel<<<blocks, threads, threads * sizeof(float)>>>(d_intermediate, d_in);
	...
	if (usesSharedMemory){
		// Launch kernel for stage-ii reduction: Shared memory size: Number of threads * data type
		shared_reduce_kernel<<<blocks, threads, threads * sizeof(float)>>>(d_out, d_intermediate);
...
int main(int argc, char **argv){
    ...
    switch(whichKernel){
    	case 0: 
	    	printf("Running reduce kernel that uses global memory.\n");
			...
			break;
		case 1:
			printf("Running reduce kernel that uses shared memory.\n");
	    	cudaEventRecord(start, 0);
		    for (int i = 0; i < 100; i++){
			...
			break;
		default:
			fprintf(stderr, "Error: No kernal was ran.\n");
			exit(EXIT_FAILURE);
    }
    ...
    elapsedTime /= 100.0f;  // 100 trial runs
